...
#  original
class NeRFWModel(torch.nn.Module):
    def __init__(
        self,
        num_layers=8,
        hidden_size=256,
        ...
        super(NeRFWModel, self).__init__()
        ...
        codesize = self.dim_dir + codesize
        self.layers_dir.append(
            torch.nn.Linear(codesize + hidden_size, hidden_size // 2)
        )
        self.fc_alpha = torch.nn.Linear(hidden_size, 1)
        self.fc_rgb = torch.nn.Linear(hidden_size // 2, 3)
        self.fc_feat = torch.nn.Linear(hidden_size, hidden_size)
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            code = torch.cat(view, code)

        feat = self.relu(self.fc_feat(x))
        alpha = self.fc_alpha(x)
        x = torch.cat((feat, code), dim=-1)
        for l in self.layers_dir:
            x = self.relu(l(x))
        rgb = self.fc_rgb(x)
        return torch.cat((rgb, alpha), dim=-1)
...
class CondNeRFModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # return torch.cat((rgb, x, alpha), dim=-1)
        return torch.cat((rgb, alpha), dim=-1)


class BasisNeRFModel(torch.nn.Module):
    def __init__(
        self,
        num_layers=8,
        hidden_size=256,
        ...
        super(BasisNeRFModel, self).__init__()
        ...
        self.fc_alpha = torch.nn.Linear(hidden_size, 1)
        self.fc_rgb = torch.nn.Linear(hidden_size // 2, 3*numbasis)
        self.fc_feat = torch.nn.Linear(hidden_size, hidden_size)
        self.relu = torch.nn.functional.relu
